...
async function initTests() {
  ...
  // callback handler with no google profile should return error
  test(file + 'Mock /googleauth?code=oauth2codehere', async (t) => {
    const fs = require('fs');
    const nock = require('nock');
    ...
    var sample_profile = null; // when no data returned
    var scope = nock('https://www.googleapis.com')
        .get('/plus/v1/people/me')
        .reply(200, sample_profile);
    ...
    try {
      var expected = 'Sorry, something went wrong, please try again.';
      var response = await server.inject(options);

      t.equal(response.statusCode, 503, "/googleauth returns 503 when there is no profile data");
      t.equal(response.payload, expected, "> " + expected);
